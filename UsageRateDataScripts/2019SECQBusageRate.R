...
cfbQBusage <- function(year){

    # by position groups
    base_url = "https://api.collegefootballdata.com/player/usage?year="
    full_url = paste0(base_url,year,"&conference=SEC&position=QB&excludeGarbageTime=true", flatten = TRUE)
    print(full_url)
...
PLOT = ggplot(chartdata1) + #data with ggplot
  ...
  scale_y_continuous(expand = c(0,0),
                     limits = c(0,1),
                     breaks = seq(0,1,.1)) + #limit on graph height and start directly at 0 #limit on graph height
  ...
  labs(y = "Overall Usage Rate",
       ...
       subtitle = "SEC Quarterbacks 2019 Season (Garbage Time Removed)") +
  #Labs adds text
  theme_grey() +
...
for(i in 1:nrow(chartdata1)){ #iterate through dataframe
  ...
  annotation_custom(grob=g[[i]], xmin=i-.5, xmax=i+.5, ymin=chartdata1$usage.overall[i], ymax=chartdata1$usage.overall[i]+.1) 
    #^adds each image. xmax and xmin - horizontal position. ymin and ymax - vertical position. 
}

#finish plotting
PLOT
ggsave(filename = file.path("/Users/walkerbasham/Desktop/FBALLstatsWithR/CFBAnalysis/UsageRate","SECQBusagerate.png"))
#print(chartdata1)
