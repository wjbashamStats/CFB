...
cfbRBusage <- function(year){

    # by position groups
    base_url = "https://api.collegefootballdata.com/player/usage?year="
    full_url = paste0(base_url,year,"&conference=SEC&position=RB&excludeGarbageTime=true", flatten = TRUE)
    print(full_url)
...
PLOT = ggplot(chartdata1) + #data with ggplot
  ...
  scale_y_continuous(expand = c(0,0),
                     limits = c(0,.5),
                     breaks = seq(0,.5,.05)) + #limit on graph height and start directly at 0 #limit on graph height
  ...
  labs(y = "Overall Usage Rate",
       ...
       subtitle = "SEC Running Backs 2019 Season (Garbage Time Removed)") +
  #Labs adds text
  theme_grey() +
...
#finish plotting
PLOT
ggsave(filename = file.path("/Users/walkerbasham/Desktop/FBALLstatsWithR/CFBAnalysis/UsageRate","SECRBusagerate.png"))
#print(chartdata1)
